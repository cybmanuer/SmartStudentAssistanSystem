...
#         alert_placeholder.text(f"STATUS : {alert_text}")

#         # Update current time and date continuously without buttons.
...
# Timer variables for studying time alert
sitting_time = 0  # Time spent studying in seconds
break_threshold = 900  # 15 minutes in seconds (900 seconds)
no_face_counter = 0  # Counter for no face detected alerts
no_face_threshold = 180  # No face detected for more than 3 minutes (180 seconds)
...
while st.session_state.running:
    try:
        ...
        # Check for faces detected in the frame
        if len(rects) > 0:
            no_face_counter = 0  # Reset no face counter
            
            sitting_time += 1  # Increment sitting time
            
            # Check if sitting time exceeds break threshold (15 minutes)
            if sitting_time >= break_threshold:
                engine.say("You've been studying for a while. It's time to take a break!")
                engine.runAndWait()
                sitting_time = 0  # Reset sitting time after alert
                
        else:
            no_face_counter += 1
            
            # Check if no face has been detected for more than the threshold time (3 minutes)
            if no_face_counter >= no_face_threshold:
                engine.say("ARE YOU THERE? I can't find you.")
                engine.runAndWait()
                no_face_counter = 0  # Reset counter after alert

        # Process detected faces for drowsiness detection
        for rect in rects:
            shape = predictor(gray, rect)
            shape_np = face_utils.shape_to_np(shape)
            ...
            if ear < earThresh:
                ...
                if count >= earFrames and not drowsiness_detected:
                    cv2.putText(frame, "DROWSINESS DETECTED", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    engine.say("Hey Please awake. Don't sleep.")
                    engine.runAndWait()
                    drowsy_frames += 1
                    drowsiness_detected = True
            else:
...
only face detection and monitring